...
while {true} do {
	...
	_sT = diag_tickTime;
	_all = allUnits + allDead + (entities "LandVehicle") + (entities "Ship") + (entities "Air");
	{
		if (_x isKindOf "Logic") exitWith {
			_x setVariable ["ocap_exclude", true];
		};
		
		_pos = (getPosATL _x) resize 2;
		_dir = getDir _x;
		_alive = alive _x;
	...
	} forEach (_all select {!(_x getVariable ["ocap_exclude", false])});
	...
	if (ocap_endCaptureOnNoPlayers && {allPlayers isEqualTo []}) exitWith {
